        ...
        if let error = error as? AWSMobileClientError {
            ...
            case .requestFailed(_ , let response, let error):
                if response?.statusCode == 401 {
                    self.showAlert(title: "Error", msg: "Token is invalid", {
                        Session.shared.signOut()
                        Switcher.updateRootVC(logined: false)
                    })
                } else {
                    self.showAlert(title: "Error", msg: error?.localizedDescription ?? "Something went wrong", dismissCompletion)
                }
            default:
                self.showAlert(title: "Error", msg: error.localizedDescription, dismissCompletion)
            }
        } else if let error = error as? ARLError {
            self.showAlert(title: "Error", msg: error.localizedDescription, dismissCompletion)
        } else {
            self.showAlert(title: "Error", msg: error.localizedDescription, dismissCompletion)
        }
