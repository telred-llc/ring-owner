...
final class RegisterViewModel: BaseObservableObject {
    //MARK: - Properties
    ...
    func register() {
        guard validate().0 else {
            //Todo: show mess
            showAlert(title: "Error", mess: validate().1, data: nil)
            return
        }
        registerToServer()
    }
    ...
    private func registerToServer() {
        guard let phoneCode = phoneModel?.phoneCode else {
            showAlert(title: "Error", mess: "Invalid phone code!", data: nil)
            return
        }
