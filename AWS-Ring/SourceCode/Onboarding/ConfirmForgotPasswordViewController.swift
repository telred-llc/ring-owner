...
class ConfirmForgotPasswordViewController: BaseViewController {
    ...
    @objc func resendCode() {
        showProgressHub()
        AWSMobileClient.sharedInstance().forgotPassword(username: self.username) { (result, error) in
    ...
    @objc func verifyCode() {
        ...
        showProgressHub()
        AWSMobileClient.sharedInstance().confirmForgotPassword(username: self.username, newPassword: (self.newPasswordInputTextFiled.text ?? ""), confirmationCode: (self.codeInputTextField.text ?? "")) { (result, error) in
            self.hideProgressHub()
            if let error = error {
                self.showErrorMessage(error: error)
            } else if let result = result {
