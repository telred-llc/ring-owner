...
class ConfirmationViewController: BaseViewController {
    ...
    func confirmSignUp() {
        self.showProgressHub()
        AWSMobileClient.sharedInstance().confirmSignUp(username: username, confirmationCode: codeTextField.text!) { (signUpResult, error) in
    ...
    func confirmSignIn() {
        AWSMobileClient.sharedInstance().confirmSignIn(challengeResponse: codeTextField.text!) { (signInResult, error) in
            if let error = error  {
                self.showErrorMessage(error: error)
